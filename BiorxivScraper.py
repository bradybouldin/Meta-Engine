...
def bioarch(searchterm):
    Results=list()
    url="https://www.biorxiv.org/search/"+searchterm+"%20numresults%3A30%20sort%3Arelevance-rank"
    ...
    for end in links:
        URL='https://www.biorxiv.org'+end
        SOURCE = urllib.request.urlopen(URL).read()
        SOUP = bs.BeautifulSoup(SOURCE,'lxml')
        data= SOUP.find('div', attrs={'class':"inside"})
        datacite=data.find('div',attrs={'class':"panel-pane pane-highwire-article-citation"}).div.div.find('div',attrs={'class':"highwire-cite highwire-cite-highwire-article highwire-citation-biorxiv-article-top clearfix has-author-tooltip"})
